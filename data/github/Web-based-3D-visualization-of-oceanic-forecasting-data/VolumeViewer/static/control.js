...
// 默认模型初始化
initialize_model();
initialize_demo_controls();

function initialize_demo_controls() {
    var demo_date = '2020-03-27';
    var demo_time = '2020-03-27 09:00';
    $('.date-select-options').each(function () {
        $(this).append('<option value="20200327">' + demo_date + '</option>');
    });
    $('.ts-select-options, #current-selected-time').each(function () {
        $(this).append('<option value="demo">' + demo_time + '</option>');
    });
    ['temp', 'salinity'].forEach(function (type) {
        [118, 119, 120, 121].forEach(function (value) {
            $('#' + type + '-selectedX').append('<option value="' + value + '">' + value + '</option>');
        });
        [18, 19, 20, 21].forEach(function (value) {
            $('#' + type + '-selectedY').append('<option value="' + value + '">' + value + '</option>');
        });
        [0, -600, -1200, -1800].forEach(function (value) {
            $('#' + type + '-selectedZ').append('<option value="' + value + '">' + value + '</option>');
        });
    });
    [118, 119, 120, 121].forEach(function (value) {
        $('#current-selectedX').append('<option value="' + value + '">' + value + '</option>');
    });
    [18, 19, 20, 21].forEach(function (value) {
        $('#current-selectedY').append('<option value="' + value + '">' + value + '</option>');
    });
    [0, -600, -1200, -1800].forEach(function (value) {
        $('#current-selectedZ').append('<option value="' + value + '">' + value + '</option>');
    });
}
...
function create_demo_volume() {
    var x = [], y = [], z = [], value = [];
    var xSize = 18, ySize = 14, zSize = 10;

    for (var depth = 0; depth < zSize; depth++) {
        for (var latitude = 0; latitude < ySize; latitude++) {
            for (var longitude = 0; longitude < xSize; longitude++) {
                var longitudeRatio = longitude / (xSize - 1);
                var latitudeRatio = latitude / (ySize - 1);
                var depthRatio = depth / (zSize - 1);
                x.push(118 + longitudeRatio * 3);
                y.push(18 + latitudeRatio * 3);
                z.push(-depthRatio * 1800);
                var horizontalVariation = 3 * Math.sin(longitudeRatio * Math.PI * 2) +
                    2 * Math.cos(latitudeRatio * Math.PI * 2) +
                    1.5 * Math.sin((longitudeRatio + latitudeRatio) * Math.PI * 3);
                value.push(Math.min(27.82, Math.max(0.99, 1 + 26 * (1 - depthRatio) +
                    horizontalVariation * (0.35 + 0.65 * (1 - depthRatio)))));
            }
        }
    }
...
function initialize_model() {
    var demo_volume = create_demo_volume();
    var temp = {
        name: 'Temperature',
        x: demo_volume.x,
        y: demo_volume.y,
        z: demo_volume.z,
        value: demo_volume.value,
        visible: true,
        type: 'volume',
        opacity: 0.5,
        colorscale: clr_bar_t,
        cmin: 0,
        cmax: 28,
        isomin: 0,
        isomax: 28,
    ...
    var salinity = {
        ...
        y: demo_volume.y,
        z: demo_volume.z,
        value: demo_volume.value.map(function (item) { return 34 + item * 0.08; }),
        visible: false,
        type: 'volume',
        opacity: 0.5,
        colorscale: clr_bar_s,
        cmin: 34,
        cmax: 37,
        isomin: 34,
        isomax: 37,
    ...
    var current = {
        ...
        y: demo_volume.y,
        z: demo_volume.z,
        value: demo_volume.value.map(function (item) { return Math.abs(item - 14) * 0.08; }),
        visible: false,
        type: 'volume',
        opacity: 0.5,
        cmin: 0,
    ...
    var current_xy = {
        ...
        y: demo_volume.y,
        z: demo_volume.z,
        value: demo_volume.value.map(function (item) { return Math.abs(item - 14) * 0.05; }),
        visible: false,
        type: 'volume',
        opacity: 0.5,
        cmin: 0,
    ...
    var current_z = {
        ...
        y: demo_volume.y,
        z: demo_volume.z,
        u: demo_volume.x.map(function () { return 0.15; }),
        v: demo_volume.x.map(function () { return 0.1; }),
        w: demo_volume.x.map(function () { return -0.05; }),
        visible: false,
        type: 'cone',
        // sizemode: 'absolute',
    ...
    function render_plot(terrain) {
        var traces = [temp, salinity, current, current_xy, current_z];
        if (terrain) {
            traces.push(terrain);
        }

        Plotly.newPlot('drawBox', traces, layout, config);
        myPlot = document.getElementById('drawBox');
        myPlot.on('plotly_click', my_click_handler); // 绑定单击事件

        $(".data-show-terrain").prop("checked", Boolean(terrain));
    }
